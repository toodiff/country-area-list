/*
    https://unstats.un.org/unsd/methodology/m49/ 爬数据
    浏览器控制台执行下列脚本, 这里只抓取了中英文, 其它语言的可以根据网页中id自行添加

    let chn = Array.from($('#CHN_COUNTRIES').find('tr')).filter((t,i)=>i);  
    let data = Array.from($('#ENG_COUNTRIES')
    .find('tr'))
    .filter((t,i)=>i)
    .map((t,i)=>{
        let tds = $(t).find('td'); 
...
export const countriesAndAreasIso = [
    {
        "name_zh": "阿富汗",
        "name_en": "Afghanistan",
        "iso_n3": "004",
